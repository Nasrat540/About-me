...
- ssh into the server
    - For Mac and Linux: `ssh root@znasrat.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo znasrat`
    - replace znasrat with your own
- create password
    - `passwd znasrat`
- Move .ssh key file
    - `cp -r .ssh /home/znasrat`
- change ownership of the .ssh directory
    - `cd /home/znasrat`
    - `chown -R znasrat:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh znasrat@znasrat.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir /var/www/znasrat.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/znasrat.com/html`
...
- `sudo vim /etc/nginx/sites-available/znasrat.com`
...
server {
    ...
    root /var/www/znasrat.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name znasrat.com www.znasrat.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/znasrat.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/znasrat/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/znasrat/.ssh/github_deploy_key.pub >> /home/znasrat/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/znasrat/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H znasrat.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* znasrat@znasrat.com:/var/www/znasrat.com/html
```
